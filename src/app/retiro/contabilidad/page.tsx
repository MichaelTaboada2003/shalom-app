...
import RifasComponent from '../rifas/page';
...
interface RaffleRecord { id: string; activityId?: string; }
interface RaffleRelation { raffleId: string; }
...
const RAFFLES_STORAGE_KEY = 'shalom-retiro-raffles';
const RAFFLE_SELLERS_STORAGE_KEY = 'shalom-retiro-raffle-sellers';
const RAFFLE_SALES_STORAGE_KEY = 'shalom-retiro-raffle-sales';
...
export default function ContabilidadPage() {
  ...
  useEffect(() => {
    let active = true;
    const legacyActivities = readStorage<AccountingActivity>(ACTIVITY_STORAGE_KEY);
    const legacyMovements = readStorage<AccountingMovement>(MOVEMENT_STORAGE_KEY).map(item => ({ ...item, activityId: item.activityId ?? null }));
    fetch('/api/retreat', { cache: 'no-store' }).then(async response => {
      if (!response.ok) throw new Error('No se pudo cargar Contabilidad');
      return response.json();
    }).then(data => {
      if (!active) return;
      setActivities(data.activities.length || !legacyActivities.length ? data.activities : legacyActivities);
      setMovements(data.movements.length || !legacyMovements.length ? data.movements : legacyMovements);
    }).catch(() => { if (active) { setActivities(legacyActivities); setMovements(legacyMovements); } }).finally(() => { if (active) setReady(true); });
    return () => { active = false; };
  }, []);

  useEffect(() => { if (ready) void persistRetreat('activities', activities); }, [activities, ready]);
  useEffect(() => { if (ready) void persistRetreat('movements', movements); }, [movements, ready]);
  ...
  const openMovementForm = useCallback((type: MovementType = 'income', activityId?: string) => {
    if (!activityId && selectedActivity?.component === 'raffles') {
  ...
  const deleteActivity = () => {
    ...
    if (activityPendingDeletion.component === 'raffles') {
      const activityRaffles = readStorage<RaffleRecord>(RAFFLES_STORAGE_KEY).filter(raffle => raffle.activityId === activityPendingDeletion.id);
      const raffleIds = new Set(activityRaffles.map(raffle => raffle.id));
      writeStorage(RAFFLES_STORAGE_KEY, readStorage<RaffleRecord>(RAFFLES_STORAGE_KEY).filter(raffle => !raffleIds.has(raffle.id)));
      writeStorage(RAFFLE_SELLERS_STORAGE_KEY, readStorage<RaffleRelation>(RAFFLE_SELLERS_STORAGE_KEY).filter(seller => !raffleIds.has(seller.raffleId)));
      writeStorage(RAFFLE_SALES_STORAGE_KEY, readStorage<RaffleRelation>(RAFFLE_SALES_STORAGE_KEY).filter(sale => !raffleIds.has(sale.raffleId)));
    }
      ...
      <section className="mt-6">
        ...
        <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {activities.map(activity => {
            ...
            return <button key={activity.id} onClick={() => setActivityFilter(activity.id)} className={`min-w-48 rounded-2xl border p-3 text-left transition-colors ${activityFilter === activity.id ? 'border-success bg-success-soft/50' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="flex items-center gap-2 text-xs font-black text-text-primary">{activity.component === 'raffles' ? <Ticket size={15} className="text-warning" /> : <FolderKanban size={15} className="text-success" />}<span className="truncate">{activity.name}</span></span><small className="mt-1 block max-w-44 truncate text-[10px] font-bold text-text-muted">{activity.component === 'raffles' ? 'Componente: Rifas' : activity.description || 'Actividad sin descripción'}</small><b className={`mt-2 block text-xs tabular-nums ${totals.balance < 0 ? 'text-danger' : 'text-success'}`}>{activity.component === 'raffles' ? 'Ver rifas' : `${formatMoney(totals.balance)} · ${totals.count} mov.`}</b></button>;
          })}
          {movements.some(item => !item.activityId) && <button onClick={() => setActivityFilter('unassigned')} className={`min-w-40 rounded-2xl border p-3 text-left transition-colors ${activityFilter === 'unassigned' ? 'border-warning bg-warning-soft' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="text-xs font-black text-text-primary">General</span><small className="mt-2 block text-[10px] font-bold text-text-muted">{totalsFor(null).count} movimientos</small></button>}
        </div>
      </section>

      {selectedActivity?.component === 'raffles' && <RifasComponent embedded activityId={selectedActivity.id} activityName={selectedActivity.name} onSettlement={movement => setMovements(current => [movement, ...current])} />}

      {selectedActivity?.component !== 'raffles' && <><section className="mt-6"><div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-base font-bold">{selectedActivity ? selectedActivity.name : activityFilter === 'unassigned' ? 'General' : 'Movimientos del retiro'}</h2><p className="mt-0.5 text-xs text-text-muted">{selectedActivity?.description || 'Registra ingresos y gastos dentro de la actividad elegida.'}</p></div><div className="flex rounded-xl border border-border-subtle bg-bg-card p-1">{([{ id: 'all', label: 'Todos' }, { id: 'income', label: 'Ingresos' }, { id: 'expense', label: 'Gastos' }] as const).map(option => <button key={option.id} onClick={() => setTypeFilter(option.id)} className={`rounded-lg px-2.5 py-1.5 text-[10px] font-black transition-colors ${typeFilter === option.id ? 'bg-accent-soft text-text-primary' : 'text-text-muted hover:text-text-secondary'}`}>{option.label}</button>)}</div></div>
