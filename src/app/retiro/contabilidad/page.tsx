...
import { ArrowDownRight, ArrowLeft, ArrowUpRight, CalendarDays, FolderKanban, Landmark, Plus, ReceiptText, Tag, Trash2, WalletCards, X } from 'lucide-react';
import { useRouter } from 'next/navigation';

type MovementType = 'income' | 'expense';
type TypeFilter = 'all' | MovementType;

interface AccountingActivity { id: string; name: string; description: string; createdAt: string; }
...
export default function ContabilidadPage() {
  ...
  const submitActivity = (event: React.FormEvent) => {
    ...
    const activity: AccountingActivity = { id: crypto.randomUUID(), name: activityName.trim(), description: activityDescription.trim(), createdAt: today() };
    setActivities(current => [activity, ...current]);
    setActivityFilter(activity.id);
    setActivityName(''); setActivityDescription(''); setShowActivityForm(false);
  };
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 pb-5 pt-4">
      ...
      <section className="mt-6"><div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-base font-bold">Actividades</h2><p className="mt-0.5 text-xs text-text-muted">Cada actividad reúne tantos movimientos como necesites.</p></div><button onClick={() => setShowActivityForm(true)} className="secondary-button min-h-9 px-3 text-xs"><Plus size={15} /> Nueva</button></div><div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"><button onClick={() => setActivityFilter('all')} className={`min-w-40 rounded-2xl border p-3 text-left transition-colors ${activityFilter === 'all' ? 'border-accent bg-accent-soft' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="flex items-center gap-2 text-xs font-black text-text-primary"><Landmark size={15} className="text-accent" /> Todo el retiro</span><small className="mt-2 block text-[10px] font-bold text-text-muted">{movements.length} movimientos · {formatMoney(balance)}</small></button>{activities.map(activity => { const totals = totalsFor(activity.id); return <button key={activity.id} onClick={() => setActivityFilter(activity.id)} className={`min-w-48 rounded-2xl border p-3 text-left transition-colors ${activityFilter === activity.id ? 'border-success bg-success-soft/50' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="flex items-center gap-2 text-xs font-black text-text-primary"><FolderKanban size={15} className="text-success" /><span className="truncate">{activity.name}</span></span><small className="mt-1 block max-w-44 truncate text-[10px] font-bold text-text-muted">{activity.description || 'Actividad sin descripción'}</small><b className={`mt-2 block text-xs tabular-nums ${totals.balance < 0 ? 'text-danger' : 'text-success'}`}>{formatMoney(totals.balance)} · {totals.count} mov.</b></button>; })}{movements.some(item => !item.activityId) && <button onClick={() => setActivityFilter('unassigned')} className={`min-w-40 rounded-2xl border p-3 text-left transition-colors ${activityFilter === 'unassigned' ? 'border-warning bg-warning-soft' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="text-xs font-black text-text-primary">Sin actividad</span><small className="mt-2 block text-[10px] font-bold text-text-muted">{totalsFor(null).count} movimientos previos</small></button>}</div></section>
