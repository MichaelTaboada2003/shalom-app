      ...
      <section className="mt-6">
        <div className="mb-3 flex items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => setShowActivityForm(true)} className="secondary-button min-h-9 px-3 text-xs"><Plus size={15} /> Nueva</button>
          </div>
        </div>
        <div className="flex gap-3 overflow-x-auto pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          ...
          {activities.map(activity => {
            ...
            return <button key={activity.id} onClick={() => setActivityFilter(activity.id)} className={`min-w-48 rounded-2xl border p-3 text-left transition-colors ${activityFilter === activity.id ? 'border-success bg-success-soft/50' : 'border-border-subtle bg-bg-card hover:border-border-medium'}`}><span className="flex items-center gap-2 text-xs font-black text-text-primary"><FolderKanban size={15} className="text-success" /><span className="truncate">{activity.name}</span></span><small className="mt-1 block max-w-44 truncate text-[10px] font-bold text-text-muted">{activity.description || 'Actividad sin descripción'}</small><b className={`mt-2 block text-xs tabular-nums ${totals.balance < 0 ? 'text-danger' : 'text-success'}`}>{formatMoney(totals.balance)} · {totals.count} mov.</b></button>;
          })}
